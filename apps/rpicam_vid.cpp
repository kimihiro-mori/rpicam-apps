...
static void event_loop(RPiCamEncoder &app)
{
	...
	const double target_fps = options->Get().framerate.value_or(30);
	...
	for (unsigned int count = 0; ; count++)
	{
		...
		if (timeout || frameout || key == 'x' || key == 'X')
		{
			if (timeout)
				LOG(1, "Halting: reached timeout of "
					<< options->Get().timeout.get<std::chrono::milliseconds>() << " milliseconds.");
			app.StopCamera();
			app.StopEncoder();
			shutdown();
		...
		if (now - last_report >= report_interval)
		{
			...
			LOG(1, "Throughput: " << std::fixed << std::setprecision(1) << measured_fps
				...
				<< "), enc queue: " << enc_q << "/" << options->Get().buffer_count
				<< " (peak " << enc_queue_peak << ")"
				<< ", drops: " << drop_count);
