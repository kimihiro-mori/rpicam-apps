...
void FileOutput::openFile(int64_t timestamp_us)
{
	...
	else if (!options_->Get().output.empty())
	{
		...
		if (options_->Get().output.find("%s") != std::string::npos)
		{
			...
			n = snprintf(filename, sizeof(filename), options_->Get().output.c_str(), ts);
		}
		else
		{
			n = snprintf(filename, sizeof(filename), options_->Get().output.c_str(), count_);
		}
		count_++;
